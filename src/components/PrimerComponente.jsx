...
export default function PrimerComponente({ shouldPlay }) {
  ...
  const loadVideos = async () => {
    setIsVideoLoading(true); // Indica que el video está cargando.
    try {
      // Cargar video grande
      ...
      setIsVideoLoading(false); // Indica que el video ha terminado de cargarse.
    }
  };
  ...
  return (
    <div className="relative w-full overflow-hidden bg-[#16222F]">
      {/* Placeholder mientras los videos cargan */}
      {isVideoLoading && (
        <div className="w-full h-full flex items-center justify-center bg-[#16222F]">
